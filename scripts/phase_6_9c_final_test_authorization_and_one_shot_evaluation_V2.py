#!/usr/bin/env python3
"""
Phase 6.9c — Final-Test Authorization + One-Shot Evaluation V2
...
SCHEMA_VERSION = "ITRS_PHASE6_9C_FINAL_TEST_V2"
...
EXPECTED_INITIAL_STATE_FILE_SHA256 = (
    "a7451fa138e440dd6dd6e56375984498"
    "2ec68126d9fc02febdc463e203ee6224"
)
...
AUTHORIZATION_PATH = Path(
    "data/experimental/phase_6/contracts/"
    "phase_6_9c_final_test_authorization_V2.json"
)
...
def load_runtime():
    phase6_base = load_module(
        PHASE6_BASE_PATH,
        "_phase6_9c_portable_base",
    )

    preflight_runtime = load_module(
        PREFLIGHT_SOURCE_PATH,
        "_phase6_9c_validation_preflight",
    )
    ...
    for symbol in (
        ...
        require(
            hasattr(phase6_base, symbol),
            f"Phase-6 portable runtime missing symbol: {symbol}",
        )
    ...
    for symbol in (
        ...
        require(
            ...
            f"Frozen validation runtime missing symbol: {symbol}",
        )
    ...
    for symbol in (
        ...
        require(
            hasattr(roundtrip, symbol),
            f"Frozen roundtrip runtime missing symbol: {symbol}",
        )
...
def construct_selected_test_model(
    *,
    phase6_base,
    roundtrip,
    checkpoint,
    ...
    require(
        file_sha256(INITIAL_STATE_PATH)
        == EXPECTED_INITIAL_STATE_FILE_SHA256,
        "Portable initial-state file SHA256 drift.",
    )

    require(
        phase6_base.EXPECTED_INITIAL_FILE_SHA
        == EXPECTED_INITIAL_STATE_FILE_SHA256,
        "Phase-6 portable initial-state file anchor drift.",
    )

    require(
        phase6_base.EXPECTED_INITIAL_MODEL_SHA
        == EXPECTED_INITIAL_MODEL_SHA256,
        "Phase-6 portable logical-model anchor drift.",
    )
    ...
    require(
        initial_payload["logical_model_sha256"]
        == EXPECTED_INITIAL_MODEL_SHA256,
        "Portable initial-state logical model SHA drift.",
    )

    preflight = roundtrip.load_preflight_runtime()

    phase6_base.install_portable_initialization_bridge(
        preflight,
    ...
    require(
        hash_fn(model)
        == EXPECTED_INITIAL_MODEL_SHA256,
        "Portable canonical constructor did not reproduce the frozen initial state.",
    )
    ...
    shared = roundtrip.load_shared_inputs(
        preflight
    )
...
def preflight_gate():
    ...
    (
        ...
        phase6_base=phase6_base,
        roundtrip=runtime_2b,
    ...
    bindings = {
        ...
        "portable_initial_model_logical_sha256": (
            EXPECTED_INITIAL_MODEL_SHA256
        ),
    ...
    print("Portable Phase-6 initialization:       PASS")
