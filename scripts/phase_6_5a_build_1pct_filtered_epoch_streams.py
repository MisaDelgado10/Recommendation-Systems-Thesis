...
SELECTION_MANIFEST_PATH = Path(
    ...
    "1pct/positive_selection_manifest_1pct.parquet"
)

OUT_DIR = Path(
    "data/experimental/phase_6/"
    "reduced_training/1pct/epoch_streams"
)
...
def main():
    ...
    banner(
        "PHASE 6.5a — BUILD EXACT FILTERED "
        "1% TRAINING STREAMS FOR EPOCHS 0..19"
    )
    ...
    print(
        "  retain selected 1% positive groups only"
    )
    print(
    ...
    roundtrip = load_module(
        ROUNDTRIP_PATH,
        "_phase6_5a_roundtrip",
    )

    generator = load_module(
        GENERATOR_PATH,
        "_phase6_5a_generator",
    )
    ...
    for epoch in range(
        ...
        banner(
            f"EPOCH {epoch:02d} — FULL STREAM -> 1% FILTER"
        )
        ...
        # Then remap full serialized indices into compact 1% serialization.
        # ---------------------------------------------------------------------
    ...
    contract = {
        ...
        "stream_construction": {
            ...
            "order_policy": (
                ...
                "order, then remap to compact 1% serialization"
            ),
    ...
    print(
        "PHASE 6.5a: PASS / "
        "20 FILTERED 1% EPOCH STREAMS FROZEN"
    )
