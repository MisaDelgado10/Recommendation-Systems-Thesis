...
SELECTION_MANIFEST_PATH = Path(
    ...
    "5pct/positive_selection_manifest_5pct.parquet"
)

OUT_DIR = Path(
    "data/experimental/phase_6/"
    "reduced_training/5pct/epoch_streams"
)
...
def main():
    ...
    banner(
        "PHASE 6.6b — BUILD EXACT FILTERED "
        "5% TRAINING STREAMS FOR EPOCHS 0..19"
    )
    ...
    print(
        "  retain selected 5% positive groups only"
    )
    print(
    ...
    roundtrip = load_module(
        ROUNDTRIP_PATH,
        "_phase6_6b_roundtrip",
    )

    generator = load_module(
        GENERATOR_PATH,
        "_phase6_6b_generator",
    )
    ...
    for epoch in range(
        ...
        banner(
            f"EPOCH {epoch:02d} — FULL STREAM -> 5% FILTER"
        )
        ...
        # Then remap full serialized indices into compact 5% serialization.
        # ---------------------------------------------------------------------
    ...
    contract = {
        ...
        "stream_construction": {
            ...
            "order_policy": (
                ...
                "order, then remap to compact 5% serialization"
            ),
    ...
    print(
        "PHASE 6.6b: PASS / "
        "20 FILTERED 5% EPOCH STREAMS FROZEN"
    )
