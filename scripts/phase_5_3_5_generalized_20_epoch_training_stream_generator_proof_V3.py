#!/usr/bin/env python3
"""
Phase 5.3.5 — Generalized 20-Epoch Training Stream Generator Proof V3
...
and the complete epoch-0 negative matrix byte-for-byte before it is trusted
for epochs 1..19. During epoch-0 regeneration, the frozen matrix is compared
row-by-row so any divergence fails at the first mismatching positive event.
"""
...
def build_first_positive_segment_index_from_phase2(
    *,
    ...
    historical["segment_number"] = pd.to_numeric(
        historical["segment_number"],
        errors="raise",
    ).astype(np.int16)

    require(
        len(historical) == 1_173_422,
        (
    ...
    require(
        len(investor_nodes) == NUM_INVESTORS,
        "Investor role-node count drift.",
    )

    require(
        len(startup_nodes) == NUM_STARTUPS,
        "Startup role-node count drift.",
    )

    investor_map = pd.Series(
        investor_nodes["node_index"].to_numpy(dtype=np.int64),
        index=investor_nodes["raw_entity_id"].astype(str),
    ).to_dict()

    startup_map = pd.Series(
        startup_nodes["node_index"].to_numpy(dtype=np.int64),
        index=startup_nodes["raw_entity_id"].astype(str),
    ).to_dict()

    investor_global = (
        historical["investor_id"]
        .astype(str)
        .map(investor_map)
    )

    startup_global = (
        historical["startup_id"]
        .astype(str)
        .map(startup_map)
    )

    require(
        bool(investor_global.notna().all()),
        "Historical investor role mapping incomplete.",
    )

    require(
        bool(startup_global.notna().all()),
        "Historical startup role mapping incomplete.",
    )

    investor_global_np = investor_global.to_numpy(
        dtype=np.int64
    )

    startup_global_np = startup_global.to_numpy(
        dtype=np.int64
    )
    ...
    require(
        bool(
            (
                (investor_global_np >= 0)
                & (investor_global_np < NUM_INVESTORS)
            ).all()
        ),
        "Historical investor index outside Investor slice.",
    )

    require(
        bool(
            (
                (startup_global_np >= NUM_INVESTORS)
                & (
                    startup_global_np
        ...
        "Historical startup index outside Startup slice.",
    )
    ...
    # Sort by pair, then segment ascending, so the first record for each
    # pair is its minimum historical segment.
    order = np.lexsort(
        (
            segment_np.astype(
                np.int64,
...
def main() -> None:
    ...
    print(
        "Authoritative source:                 PHASE-2 TEMPORAL"
    )
    print(
