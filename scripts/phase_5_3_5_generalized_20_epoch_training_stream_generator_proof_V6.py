#!/usr/bin/env python3
"""
Phase 5.3.5 — Generalized 20-Epoch Training Stream Generator Proof V6
...
PRECLOSURE_SNAPSHOT_PATH = (
    AUDIT_DIR
    / "phase_5_3_5_preclosure_generation_snapshot.json"
)
...
def main() -> None:
    ...
    # ---------------------------------------------------------------------
    # Persist scalar representation facts before freeing the 1,073,249 x 4
    # epoch-0 matrix. All later invariants/contracts MUST use these scalar
    # facts rather than touching freed array memory.
    # ---------------------------------------------------------------------

    generated_negative0_dtype = np.dtype(
        generated_negative0.dtype
    )

    generated_negative0_dtype_str = str(
        generated_negative0.dtype.str
    )

    generated_negative0_c_contiguous = bool(
        generated_negative0.flags.c_contiguous
    )

    generated_negative0_f_contiguous = bool(
        generated_negative0.flags.f_contiguous
    )

    generated_negative0_shape = tuple(
        int(value)
        for value
        in generated_negative0.shape
    )

    generated_negative0_value_exact = bool(
        exact_negative0_array
    )
    ...
    # =========================================================================
    # DURABLE PRE-CLOSURE GENERATION SNAPSHOT
    # =========================================================================

    banner(
        "WRITE DURABLE PRE-CLOSURE GENERATION SNAPSHOT"
    )
    ...
    # Save the generated stream registry and deterministic prefixes NOW,
    # before final reporting/invariant closure. This prevents an audit-only
    # failure after expensive generation from discarding all useful evidence.
    ...
    preclosure_snapshot = {
        ...
        "status": (
            "GENERATION_COMPLETE_PENDING_FINAL_CLOSURE"
        ),
        "epoch0": {
            "positive_order_sha256": (
                positive_sha
            ...
            "negative_dtype": (
                str(
                    generated_negative0_dtype
                )
            ),
            "negative_dtype_str": (
                generated_negative0_dtype_str
            ),
            "negative_shape": list(
                generated_negative0_shape
            ),
            "negative_c_contiguous": (
                generated_negative0_c_contiguous
            ),
            "negative_f_contiguous": (
                generated_negative0_f_contiguous
            ),
            "negative_values_exact_frozen": (
                generated_negative0_value_exact
            ),
            ...
            "duplicate_candidate_rejections": int(
                metadata0[
                    "duplicate_candidate_rejections"
                ]
            ),
        ...
        "distinct_negative_hashes_epoch0_1_19": (
            True
        ),
        "distinct_order_hashes_epoch0_1_19": (
            True
        ),
        "model_instantiated": (
            False
    ...
    # Convert NumPy scalar values nested in epoch rows to JSON-native values.
    ...
    PRECLOSURE_SNAPSHOT_PATH.write_text(
        json.dumps(
            _json_native(
                preclosure_snapshot
            ),
    ...
    print(
        f"WROTE  {EPOCH_REGISTRY_PATH}"
    )
    print(
        f"WROTE  {DETERMINISM_PATH}"
    )
    print(
        f"WROTE  {EPOCH1_NEGATIVE_PREFIX_PATH}"
    )
    print(
        f"WROTE  {EPOCH19_NEGATIVE_PREFIX_PATH}"
    )
    print(
        f"WROTE  {EPOCH1_ORDER_PREFIX_PATH}"
    )
    print(
        f"WROTE  {EPOCH19_ORDER_PREFIX_PATH}"
    )
    print(
        f"WROTE  {PRECLOSURE_SNAPSHOT_PATH}"
    )
    ...
    checks = [
        ...
        (
            "generated_epoch0_negative_c_contiguous",
            (
                generated_negative0_c_contiguous
                is True
            ),
        ...
        (
            "generated_epoch0_negative_shape_exact",
            (
                generated_negative0_shape
                == (
                    POSITIVE_COUNT,
        ...
        (
            "generated_epoch0_negative_values_exact_frozen",
            (
                generated_negative0_value_exact
                is True
            ),
    ...
    decision_df = pd.DataFrame(
        [
            ...
            },
            {
                ...
                "value": (
                    "CAPTURE_SCALAR_EPOCH0_REPRESENTATION_FACTS_"
                    "BEFORE_FREEING_LARGE_ARRAY"
                ),
                "classification": (
                    "AUDIT_RUNTIME_BOOKKEEPING"
                ),
                "status": (
                    "FROZEN_PHASE_5_3_5"
                ),
            ...
            {
                "decision": (
                    "durable_preclosure_snapshot"
                ),
                "value": (
                    "WRITE_STREAM_HASHES_DIAGNOSTICS_AND_PREFIXES_"
                    "BEFORE_FINAL_CLOSURE_ASSERTIONS"
                ),
                "classification": (
                    "AUDIT_RUNTIME_BOOKKEEPING"
                ),
