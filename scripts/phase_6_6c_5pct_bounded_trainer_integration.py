...
POSITIVE_PATH = Path(
    ...
    "5pct/positive_order_5pct.parquet"
)
...
OUT_DIR = Path(
    ...
    "phase_6_6c_5pct_bounded_trainer"
)
...
def save_probe_checkpoint(
    *,
    ...
    payload = {
        "schema_version": (
            "ITRS_PHASE6_5PCT_CHECKPOINT_V1"
        ),
...
def main():
    ...
    q4 = json.loads(
        PHASE6_6A_CONTRACT_PATH.read_text(
            encoding="utf-8"
        )
    )

    q5a = json.loads(
        PHASE6_6B_CONTRACT_PATH.read_text(
            encoding="utf-8"
        )
    )
    ...
    print(
        "Phase 6.6a subset gate:             PASS"
    )
    print(
        "Phase 6.6b epoch-stream gate:      PASS"
    )
    ...
    print(
        "PHASE 6.6c: PASS / "
        "5% TRAINER INTEGRATION QUALIFIED"
    )
